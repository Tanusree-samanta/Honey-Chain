...
import { BeekeeperTab, UserAccount, HiveTelemetry } from '../../types';
...
export const BeekeeperTopBar: React.FC<BeekeeperTopBarProps> = ({
  ...
  const criticalAlerts = hives.filter(h => h.status === 'critical');

  const filteredHives = searchQuery.trim()
    ? hives.filter(h => h.code.toLowerCase().includes(searchQuery.toLowerCase()) || h.name.toLowerCase().includes(searchQuery.toLowerCase()) || h.location.toLowerCase().includes(searchQuery.toLowerCase()))
    : [];
    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-[#d8c3ad]/30 px-4 sm:px-6 py-3.5 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="lg:hidden p-2 rounded-xl text-[#534434] hover:bg-[#f6ece6] transition-colors"
        >
        ...
        <div className="hidden sm:flex items-center gap-2 bg-[#adedd3]/40 border border-[#2b6954]/20 rounded-full px-3 py-1 text-xs font-bold text-[#004e34]">
          ...
          <span>LoRaWAN Gateway Online • 12 Hives Syncing</span>
        </div>
      </div>
      ...
      <div className="relative flex-1 max-w-xs sm:max-w-md">
        <div className="relative flex items-center">
          ...
          <input
            ...
            placeholder="Search hive ID, location, telemetry..."
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-2.5 text-[#867461] hover:text-[#1f1b17]"
            >
        ...
        {showSearchDropdown && searchQuery.trim() && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-[#d8c3ad]/50 rounded-2xl shadow-xl p-2 z-50 animate-in fade-in">
            <div className="text-[10px] font-bold text-[#867461] uppercase tracking-wider px-3 py-1">
              Matching Hives & Nodes
            </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button
          ...
          className="relative p-2 rounded-xl text-[#534434] hover:bg-[#f6ece6] transition-colors"
          title="Apiary Alerts"
        >
        ...
        <div className="lg:hidden fixed inset-x-0 top-[60px] bg-[#1b4332] text-white p-5 border-b border-white/10 shadow-2xl z-40 animate-in slide-in-from-top duration-200">
          <div className="flex justify-between items-center pb-3 mb-3 border-b border-white/10">
            ...
            <button
              ...
              className="text-xs font-bold text-[#95d3ba] hover:underline flex items-center gap-1"
            >
